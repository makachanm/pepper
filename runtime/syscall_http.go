...
func doSyscallHttp(vmInstance VM, code int64) {
	...
	case 400: // http_get
		...
		vmInstance.OperandStack.Push(VMDataObject{Type: STRING, StringData: string(body)})
	case 401: // http_post
		...
		vmInstance.OperandStack.Push(VMDataObject{Type: STRING, StringData: string(respBody)})
