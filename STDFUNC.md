...
#### readln
`readln` 함수는 표준 입력에서 한 줄을 읽어와 문자열로 반환한다.
```
dim line = readln[]
```

#### read_file
`read_file` 함수는 파일 경로를 인자로 받아서 파일의 내용을 문자열로 반환한다.
```
dim content = read_file[`path/to/file.txt`]
```

#### write_file
`write_file` 함수는 파일 경로와 내용을 인자로 받아서 파일에 내용을 쓴다. 성공 시 `true`를, 실패 시 `false`를 반환한다.
```
...
#### str_split
`str_split` 함수는 문자열과 구분자를 인자로 받아서 문자열을 구분자로 나눠 팩(pack)으로 반환한다.
```
dim parts = str_split[`hello,world`, `,`]
```

#### str_join
`str_join` 함수는 팩(pack)과 구분자를 인자로 받아서 팩의 요소들을 구분자로 이어붙여 문자열로 반환한다.
```
...
#### str_contains
`str_contains` 함수는 문자열과 부분 문자열을 인자로 받아서 부분 문자열이 포함되어 있는지 여부를 불리언(boolean) 값으로 반환한다.
```
dim found = str_contains[`hello world`, `world`]
```

#### str_has_prefix
`str_has_prefix` 함수는 문자열과 접두사를 인자로 받아서 문자열이 접두사로 시작하는지 여부를 불리언(boolean) 값으로 반환한다.
```
dim starts = str_has_prefix[`hello world`, `hello`]
```

#### str_has_suffix
`str_has_suffix` 함수는 문자열과 접미사를 인자로 받아서 문자열이 접미사로 끝나는지 여부를 불리언(boolean) 값으로 반환한다.
```
dim ends = str_has_suffix[`hello world`, `world`]
```

#### str_to_lower
`str_to_lower` 함수는 문자열을 인자로 받아서 모든 문자를 소문자로 바꾼 문자열을 반환한다.
```
dim lower = str_to_lower[`HELLO WORLD`]
```

#### str_to_upper
`str_to_upper` 함수는 문자열을 인자로 받아서 모든 문자를 대문자로 바꾼 문자열을 반환한다.
```
dim upper = str_to_upper[`hello world`]
```

#### str_trim
`str_trim` 함수는 문자열을 인자로 받아서 양쪽 공백을 제거한 문자열을 반환한다.
```
dim trimmed = str_trim[`  hello world  `]
```

#### str_index_of
`str_index_of` 함수는 문자열과 부분 문자열을 인자로 받아서 부분 문자열이 처음 나타나는 위치를 반환한다. 부분 문자열이 없으면 -1을 반환한다.
```
